...
import { LevelEnum } from './filter_level.enum';
...
interface RoleFilterValue {
  data: FilterData[] | FilterData | null;
  condition?: string;
}

interface RoleFilterData {
  [key: string]: RoleFilterValue;
}

interface SimplifiedFilterValue {
  data: string[] | string;
  condition?: string | undefined;
}
@Injectable()
export class RoleService {
  private filterRepositoryMap = {
    level: LevelEnum,
    office: this.officeRepository,
    desk: this.deskRepository,
  };
  constructor(
    ...
    @InjectRepository(RoleFilterRel)
    private readonly roleFilterRelRepository: Repository<RoleFilterRel>,
  ...
  async createRole(data: CreateRoleDto, user: User) {
    const maxId = await this.roleRepository
      .createQueryBuilder('role')
      .select('MAX(role.id)', 'max')
      .getRawOne();

    const newId = (maxId?.max || 0) + 1;
    const defaultDashboard = await this.customStatusRepository.findOne({
        where:{
          type:StatusType.DASHBOARD,
          name:"General Dashboard"
        }
    });
    if(!defaultDashboard){
      throw new BadRequestException("Default Dashboard Not Found");
    };
    // const count = await this.roleRepository.count();
    const result = this.roleRepository.create(
      await this.roleRepository.save({ id: newId, ...data, defaultDashboard:defaultDashboard.name, dashboardId: defaultDashboard.id}),
    );
    const permissions = await this.permissionRoleRelRepository.find({
      where: { role: { id: result.clonedFrom } },
      relations: ['permission'],
      select: { permission: { id: true, name: true } },
    });
    const newRoles = permissions.map((p) => {
      return this.permissionRoleRelRepository.create({
        permission: {
          id: p.permission.id,
        },
        role: {
    ...
    const clonedFrom = await this.roleRepository.findOne({
      where: { id: checkRole?.clonedFrom },
    });
    const department = this.getDepartmentList().find(
      (dept) => dept.id === Number(checkRole?.departmentId),
    );
    await this.permissionRoleRelRepository.save(newRoles),
      this.eventEmitter.emit(EventTypes.USER_LOG, {
        newData: {
          ...data,
          departmentId: department?.name,
          clonedFrom: clonedFrom?.name,
        },
        oldData: null,
        entityId: result.id,
        entityType: 'Role',
  ...
  async getRoles({ userId, query }: { userId: number; query: getRoleDto }) {
    return await this.roleRepository.advanceFilters({
      limit: query.limit,
      page: query.page,
      listName: ListNames.ROLE,
      userId,
  ...
  async getAllRoleFilter() {
    const data = await this.roleFilterRepository.find();
    const orderedNames = ["Level", "Desk", "Office"];
    return orderedNames.map(name => data.find(item => item.name === name));
  }

  async getAllRoleFilterById(id: number) {
    const filter = await this.roleFilterRelRepository.find({
      where: { role: { id } },
      relations: ['roleFilter'],
    });

    let officeData: Office[] = [];
    let levelData;
    let deskData;

    const office = filter.find((val) => val.roleFilter.name === 'Office');

    if (office) {
      officeData = await this.officeRepository.find({
        where: { id: In(JSON.parse(office?.filterRefIds)) },
      });
    }

    const level = filter.find((val) => val.roleFilter.name === 'Level');
    if (level) {
      levelData = LevelEnum.find(
        (val) => val.id === Number(JSON.parse(level.filterRefIds)[0]),
      );
    }

    const desk = filter.find((val) => val.roleFilter.name === 'Desk');
    if (desk) {
      deskData = await this.deskRepository.find({
        where: { id: In(JSON.parse(desk?.filterRefIds)) },
      });
    }

    return {
      level: { data: levelData },
      office: { data: officeData, condition: office?.condition },
      desk: { data: deskData, condition: desk?.condition },
    };
  }

  async getRolePermission(): Promise<Permission[]> {
    // return [
    //   {
    //     key: 'MASTER_ROLE',
    //     category: 'System',
    ...
    //     description: 'Roles with this permission allows to see kycNote column',
    //     readOnly: false,
    //   },
    //   {
    ...
    //     description: 'Can Override Validation For Force Cancel BrokerBankings',
    //     readOnly: true,
    //   },
    //   {
    ...
    //     description: 'Roles with this permission can bulk reset all TOTP keys',
    //     readOnly: false,
    //   },
    //   {
    ...
    //     subCategory: 'Miscellaneous',
    //     name: 'See Helpdesk',
    //     description: 'Roles with this permission can view the helpdesk button',
    //     readOnly: false,
    //   },
    //   {
    //     key: 'READ_REPORTS',
  ...
  async getPermissionCategory(): Promise<PermissionCategory[]> {
    // return [
    //   {
    //     id: 1,
    //     name: 'General',
  ...
  async RolesWithPermission(id: number) {
    const data = await this.repository.findOne({
      where: { id: id },
      relations: ['permissionRoleRel.permission'],
      select: {
        permissionRoleRel: {
          id: true,
          meta: true,
    ...
    // const rolePermission = await this.permissionRoleRelRepository.find({
    //   where: { role: { id } },
    //   relations: ['permission'],
    // });
    ...
    const transformedData = {
      ...data,
      permissionRoleRel: Array.isArray(data.permissionRoleRel)
        ? data.permissionRoleRel
    ...
    if (Array.isArray(data.permissionRoleRel)) {
      for (const element of data.permissionRoleRel) {
        const index = transformedData.permissionRoleRel.findIndex(
          (val) => val.id === element.permission.id,
        );
        transformedData.permissionRoleRel[index].meta = element.meta
  ...
  async getPermissionEndpoint(id: number) {
    const arr: any[] = [];
    const data = await this.repository
      .createQueryBuilder('role')
      .leftJoinAndSelect('role.permissionRoleRel', 'permissionRoleRel')
      .leftJoinAndSelect('permissionRoleRel.permission', 'permission')
      .leftJoinAndSelect(
        'permission.permissionEndpointRel',
        'permissionEndpointRel',
      )
      .leftJoinAndSelect(
        'permissionEndpointRel.permissionEndpoint',
        'permissionEndpoint',
      )
      .where('role.id = :id', { id })
      .andWhere('permissionEndpoint.isScreen = :isScreen', { isScreen: true })
      .select([
        'role.id',
        'role.name',
        'role.appName',
        ...
        'role.isHidden',
        'role.canSeeEmail',
        'role.canSeePhoneNumber',
        'role.seeOtherConfidentialData',
        'role.clonedFrom',
        'role.isActive',
        ...
        'role.createdAt',
        'role.updatedAt',
        'permissionRoleRel.id',
        'permission.id',
        'permission.key',
        'permission.name',
        'permissionEndpointRel.id',
        'permissionEndpoint.id',
        'permissionEndpoint.name',
        'permissionEndpoint.url',
        'permissionEndpoint.screen',
        'permissionEndpoint.description',
        'permissionEndpoint.type',
      ])
      .getOne();
    ...
    //   ...data,
    //   permissionRoleRel: Array.isArray(data.permissionRoleRel)
    //     ? data.permissionRoleRel.map((roleRel) => ({
    //         ...roleRel,
    //         permission: {
    //           ...roleRel.permission,
    //           permissionEndpointRel: Array.isArray(
    //             roleRel.permission.permissionEndpointRel,
    //           )
    //             ? roleRel.permission.permissionEndpointRel.map(
    //                 (endpointRel) => ({
    //                   id: endpointRel.permissionEndpoint.id,
    //                   name: endpointRel.permissionEndpoint.name,
    //                   url: endpointRel.permissionEndpoint.url,
    //                   screen: endpointRel.permissionEndpoint.screen,
    //                   description: endpointRel.permissionEndpoint.description,
    //                 }),
    //               )
    //             : [],
    //         },
    ...
    const transformedData = {
      ...data,
      permissionRoleRel: Array.isArray(data.permissionRoleRel)
        ? data.permissionRoleRel.map(({ permission }) => ({
          permissionEndpointRel: permission.permissionEndpointRel.map(
            ({ permissionEndpoint }) => arr.push({ ...permissionEndpoint }),
          ),
        }))
        : [],
    };
    ...
    // @ts-ignore
    delete transformedData.permissionRoleRel;
    // eslint-disable-next-line
    // @ts-ignore
    transformedData.permissions = arr;
    return transformedData;
  }

  async rolePermission(data: CreateRolePermissionDto , user: User) {
    const i18n = I18nContext.current();
    const { permissionId, roleId } = data;

    const permission = permissionId.map((permissionId) => ({
      id: permissionId,
    }));
    ...
    if (!role) {
      const meesage = i18n?.t('errors.role.notFound');
    ...
    const permissionExist: Permission[] = await this.permissionRepository.find({
      where: permission,
    });

    if (!permissionExist) {
      const message = i18n?.t('errors.permission.notFound');
      throw new HttpException(
        {
          status: HttpStatus.UNPROCESSABLE_ENTITY,
          error: {
    ...
    const permissionRoleRel = permissionExist.map((value) => ({
      permission: {
        id: value.id,
      },
      role: {
    ...
    return await this.permissionRoleRelRepository.save(permissionRoleRel);
  }

  async updateRolePermission(id: number, data: CreateRolePermissionDto, user: User) {
    const i18n = I18nContext.current();
    const { permissionId, roleId } = data;
    ...
    const oldPermission = await this.RolesWithPermission(roleId)
    const simplifiedOldPermissions ={permissionRoleRel: oldPermission?.permissionRoleRel.map(permission => permission.name)}
    console.log(simplifiedOldPermissions)
    if (!permissionExist) {
      const message = i18n?.t('errors.permission.notFound');
      throw new HttpException(
        {
    ...
    if (data.meta) {
      for await (const element of data.meta) {
        const widget = permissionExist.findIndex(
          (val) => val.key === element.id,
        );
        permissionExist[widget].meta = JSON.stringify(element);
      }
    }
    ...
    await this.permissionRoleRelRepository.delete({
      role: { id: id },
    });
    ...
    for (const user of users) {
      this.cacheManager.del(`get-me-api-${user.id}`);
    }
    ...
    await Promise.all(
      users.map((user) => this.cacheManager.del(`get-me-api-${user.id}`)),
    );
    ...
    const savedPermission = await this.permissionRoleRelRepository.save(permissionRoleRel)
    const newPermission = await this.RolesWithPermission(roleId)
    const simplifiedNewPermissions = {permissionRoleRel : newPermission?.permissionRoleRel.map(permission => permission.name)}
    console.log(simplifiedNewPermissions)
    await this.eventEmitter.emit(EventTypes.USER_LOG, {
      newData: {
        ...simplifiedNewPermissions,
      },
      oldData : {
        ...simplifiedOldPermissions,
      },
      entityId: roleId,
      entityType: 'Role',
      performerId: user?.id,
      performerType: 'Operator',
      field: 'Permissions Updated',
    });
    return savedPermission;
  }

  async getDashboardWidget(id: number) {
    const widget = await this.permissionRoleRelRepository.find({
      where: { role: { id } },
    });

    const widgetArr: object[] = [];

    for await (const element of widget) {
      if (element.meta) {
        widgetArr.push(JSON.parse(element.meta));
      }
    }
  ...
  // async roleFilterRel(data: CreateRoleFilterRelDto) {
  //   const i18n = I18nContext.current();
  //   const { roleFilter, roleId } = data;
  ...
  //   const findExist = await this.roleFilterRelRepository.find({
  //     where: { role: { id: roleId } },
  //   });

  //   if (findExist.length > 0) {
  //     await this.roleFilterRelRepository.delete({ role: { id: roleId } });
  //   }

  //   const obj = Object.keys(roleFilter);

  //   const filterRoleRel = obj.map((vale) => ({
  //     roleFilter: {
  //       id: Number(vale),
  //     },
  //     role: {
  //       id: roleId,
  //     },
  //     filterRefIds: `[${roleFilter[vale]}]`,
  //   }));

  //   return await this.roleFilterRelRepository.save(filterRoleRel);
  // }

  async roleFilterRel(data: CreateRoleFilterRelDto, user: User) {
    const i18n = I18nContext.current();
    const { roleId, filters } = data;

    const role = await this.roleRepository.findOne({ where: { id: roleId } });

    if (!role) {
      const message = i18n?.t('errors.role.notFound');
      throw new HttpException(
        {
          status: HttpStatus.UNPROCESSABLE_ENTITY,
          error: {
    ...
    // Delete all existing relations for this role
    const oldRole = await this.getAllRoleFilterById(roleId);
    await this.roleFilterRelRepository.delete({ role: { id: roleId } });
    console.log(oldRole)

    // Create new relations
    const newRelations: RoleFilterRel[] = [];

    for (const filter of filters) {
      const { roleFilterId, filterRefIds, condition } = filter;

      const roleFilter = await this.roleFilterRepository.findOne({
        where: { id: roleFilterId },
      });

      if (!roleFilter) {
        const message = i18n?.t('errors.roleFilter.notFound');
        throw new HttpException(
          {
            status: HttpStatus.UNPROCESSABLE_ENTITY,
            error: {
      ...
      await this.cacheManager.del(`ROLE_FILTER:${roleId}`);

      const newRoleFilterRel = this.roleFilterRelRepository.create({
        role: { id: roleId },
        roleFilter: { id: roleFilterId },
        filterRefIds,
        condition,
      });

      newRelations.push(newRoleFilterRel);
    }
    const savedRoles = await this.roleFilterRelRepository.save(newRelations);
    const newRole = await this.getAllRoleFilterById(roleId);
    const simplifiedOldRole = await this.logsRoleData(oldRole);
    const simplifiedNewRole = await this.logsRoleData(newRole);
    console.log(newRole)
    await this.eventEmitter.emit(EventTypes.USER_LOG, {
      newData: {
        ...simplifiedNewRole,
  ...
  async getRoleList(query?: GetListDto): Promise<Role[]> {
    return await this.roleRepository.find({
      where: {
        isActive: true,
        isHidden: query?.isHidden === 'true' ? true : undefined,
      },
  ...
  async getById(id: number) {
    const i18n = I18nContext.current();
    const data = await this.roleRepository.findOneBy({ id, isActive: true });
    if (!data) {
      const message = i18n?.t('errors.role.notFound');
      throw new HttpException(
        {
  ...
  async updateRole(id: number, body: UpdateRoleDto, user: User) {
    const i18n = I18nContext.current();
    const defaultRole = await this.roleRepository.findOneBy({ id });
    const clonedFromDefault = await this.roleRepository.findOne({
      where: { id: defaultRole?.clonedFrom },
    });
    const departmentDefault = this.getDepartmentList().find(
      (dept) => dept.id === Number(defaultRole?.departmentId),
    );
    if (defaultRole?.isReadOnly === true) {
      const message = i18n?.t('errors.role.defaultRoleNotUpdated');
      throw new HttpException(
        {
          status: HttpStatus.NOT_FOUND,
          error: {
    ...
    if(body.dashboardId){
      const isDashboard = await this.customStatusRepository.findOne({
        where:{
          id:body.dashboardId,
          type:StatusType.DASHBOARD
        }
      ...
      if(!isDashboard){
        throw new BadRequestException("Invalid dashboard id")
      }
      defaultDashboard = isDashboard.name;
    }
    await this.roleRepository.update(id, {...body,defaultDashboard});
    const result = await this.roleRepository.findOne({ where: { id } });
    const clonedFrom = await this.roleRepository.findOne({
      where: { id: result?.clonedFrom },
    });
    const department = this.getDepartmentList().find(
      (dept) => dept.id === Number(result?.departmentId),
    );
    this.eventEmitter.emit(EventTypes.USER_LOG, {
      newData: {
        ...result,
      ...
      oldData: {
        ...defaultRole,
        departmentId: departmentDefault?.name,
        clonedFrom: clonedFromDefault?.name,
      },
      entityId: result?.id,
      entityType: 'Role',
      performerId: user?.id,
  ...
  async deleteRole(id: number , user: User) {
    const i18n = I18nContext.current();
    const userRole = await this.userRepository.findOneBy({ role: { id } });
    if (userRole) {
      throw new HttpException(
        {
          status: HttpStatus.UNPROCESSABLE_ENTITY,
          error: {
            msg: `Role is assign to this user: ${userRole.email}`,
          },
        },
        HttpStatus.UNPROCESSABLE_ENTITY,
      );
    ...
    const defaultRole = await this.roleRepository.findOneBy({ id });
    if (defaultRole?.isReadOnly === true) {
      const message = i18n?.t('errors.role.unableToDeleteRole');
      throw new HttpException(
        {
          status: HttpStatus.NOT_FOUND,
          error: {
    ...
    await this.permissionRoleRelRepository.delete({ role: { id } });
    await this.eventEmitter.emit(EventTypes.USER_LOG, {
      newData: {
        ...defaultRole,
      },
  ...
  // async roleFilterData(roleId: number) {
  //   const roleFilter = await this.roleFilterRelRepository.find({
  //     where: { role: { id: roleId } },
  //     relations: ['roleFilter', 'role'],
  //   });

  //   const results: FilterResult[] = [];

  //   for (const roleFilters of roleFilter) {
  //     const filterName = roleFilters.roleFilter.name.toLowerCase();
  //     const repository = this.filterRepositoryMap[filterName];

  //     if (repository) {
  //       const filterRefIds = JSON.parse(roleFilters.filterRefIds);

  //       if (filterName === 'level') {
  //         const filteredData = filterRefIds
  //           .map((id) => {
  //             const levelItem = LevelEnum.find((item) => item.id === id);
  //             return levelItem
  //               ? { id: levelItem.id, name: levelItem.name }
  //               : null;
  //           })
  //           .filter((item) => item !== null);

  //         results.push({
  //           filterName: roleFilters.roleFilter.name,
  //           data: filteredData,
  //           condition: roleFilters.condition,
  //         });
  //       } else {
  //         // Handle other filters
  //         const filteredData = await repository.find({
  //           where: { id: In(filterRefIds) },
  //         });
  ...
  //   return results;
  // }
  async roleFilterData(roleId: number): Promise<RoleFilterQueryData> {
    ...
    const filterData: RoleFilterQueryData = {};
  ...
  async logsRoleData (roleData: RoleFilterData){
    const simplified = {};
    
    for (const [key, value] of Object.entries(roleData)) {
      if (!value?.data) continue;
  
      const filterValue: SimplifiedFilterValue = {
        data: Array.isArray(value.data) 
          ? value.data.map(item => item.name)
          : (value.data as FilterData).name
      };
  
      // Only add condition if it exists
      if (value.condition) {
        filterValue.condition = value.condition;
      }
  
      simplified[key] = filterValue;
    }
